...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Post.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Post.query(buildQuery({ model: Post, filters }))
  ...
  count: (params) => {
    ...
    return Post.query(buildQuery({ model: Post, filters: _.pick(filters, 'where') })).count();
  },
